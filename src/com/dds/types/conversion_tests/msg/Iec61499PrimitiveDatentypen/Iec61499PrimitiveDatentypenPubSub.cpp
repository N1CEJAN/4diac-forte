...
bool conversion_tests::Iec61499PrimitiveDatentypenPubSub::publish(CIEC_STRUCT* data) {
  const auto ciec = dynamic_cast<CIEC_Iec61499PrimitiveDatentypen *>(data);
  msg::Iec61499PrimitiveDatentypen dds = ciec2dds(*ciec);
  return this->write(&dds);
}

std::optional<std::unique_ptr<CIEC_STRUCT>> conversion_tests::Iec61499PrimitiveDatentypenPubSub::receive() {
    conversion_tests::msg::Iec61499PrimitiveDatentypen dds;
    bool taken;
    this->take(&taken, &dds);
    if (!taken) return std::nullopt;

    auto ciec = std::make_unique<CIEC_Iec61499PrimitiveDatentypen>(dds2ciec(dds));
...
CIEC_Iec61499PrimitiveDatentypen conversion_tests::Iec61499PrimitiveDatentypenPubSub::dds2ciec(
  ...
  ciec.var_a_word = CIEC_WORD(dds.a_word());
  ciec.var_a_dword = CIEC_DWORD(dds.a_dword());
  ciec.var_a_lword = CIEC_LWORD(dds.a_lword());
